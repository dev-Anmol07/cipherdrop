...
# and can reset on a restart too, silently wiping this database while
# browsers keep their vault identities in localStorage forever. That
# mismatch is exactly what produces "No Vault found" for an identity
# Settings still shows as valid. Point DB_PATH at a Render persistent
# disk's mount path (Render dashboard -> service -> Disks) via this env
# var so the database survives deploys; falls back to a local file
# alongside the app for local dev, where that's not a concern.
DB_PATH = Path(os.environ.get("CIPHERDROP_DB_PATH", str(BASE_DIR / "cipherdrop.db")))

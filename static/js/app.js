...
(() => {
  ...
  // ---- Auto-Vault: silent identity on first visit ----
  // Zero-friction onboarding: instead of making "quantum-safe by default"
  // depend on a user finding Settings and filling out a form, generate a
  // permanent identity the moment someone shows up with none, entirely in
  // the background. No prompt, no choice, nothing to fill in — if they
  // never open Settings they'll never know this ran. It only ever creates
  // an identity when none exists yet, so it can't clobber one the person
  // (or an import) already set up.
  function randomVaultId() {
    const hex = crypto.randomUUID().replace(/-/g, '').slice(0, 10);
    return `guest-${hex}`;
  }

  async function ensureAutoVaultIdentity() {
    if (loadIdentity()) return; // already has one — manual, imported, or a prior auto-vault
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        ...
        if (res.ok) {
          saveIdentity({ vaultId, ek: bytesToB64(ek), dk: bytesToB64(dk) });
          renderIdentityPanel();
          return;
        }
        if (res.status !== 409) return; // not a name collision — don't hammer the server, just skip for now
        // 409: astronomically unlikely random-ID collision — try once more with a fresh one.
      } catch {
        return; // offline / network hiccup — next page load will retry from scratch
      }
    }
  }

  // A saved identity can outlive the server's record of it — a redeploy,
  // a disk reset, a DB migration — leaving the browser still holding a
  // vault ID and keypair the server has never heard of. Nothing detects
  // that on its own: Settings still shows the identity as if all were
  // well, and the first sign of trouble is a "No Vault found" error on
  // a send, including sending to yourself. Confirm the server still
  // recognizes the saved public key on load, and if not, re-publish the
  // same keypair — same fingerprint, so it's a no-op for anyone who
  // already trusts it, just restoring the server's copy.
  async function reconcileVaultIdentity() {
    const identity = loadIdentity();
    if (!identity) return;
    try {
      const res = await fetch(`/api/vault/${encodeURIComponent(identity.vaultId)}/pubkey`);
      if (res.ok) return; // server already knows about it
      if (res.status !== 404) return; // some other error — don't act on a guess

      const reReg = await fetch('/api/vault/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ vaultId: identity.vaultId, ek: identity.ek }),
      });
      if (reReg.ok) renderIdentityPanel();
      // A 409 here means someone else claimed the name while the server
      // had forgotten it — no safe automatic move; it'll surface next
      // time the person opens Settings.
    } catch {
      // offline — next page load tries again
    }
  }
  ...
  function main() {
    ...
    // Fire-and-forget: never blocks first paint or any of the inits
    // above on a network round trip. renderIdentityPanel() gets called
    // again once it resolves, in case Settings is already open.
    ensureAutoVaultIdentity().then(reconcileVaultIdentity);

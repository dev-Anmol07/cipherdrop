...
3. Configure:
   ...
   - **Start Command:** `gunicorn app:app --worker-class gthread --workers 1 --threads 4 --bind 0.0.0.0:$PORT`
     (matches the `Procfile` — stick to 1 worker unless you've also
     pointed `RATELIMIT_STORAGE_URI` at Redis; see the note below on why)
   - **Instance Type:** Free
